...
TestExportStructurePython = {} -- class

function TestExportStructurePython:setUp()
    local c0 = nbp.Node:new('C', nbp.T_CLASS, 0)
    local c2 = nbp.Node:new('C', nbp.T_CLASS, 2)
    local c4 = nbp.Node:new('C', nbp.T_CLASS, 4)
    local f0 = nbp.Node:new('F', nbp.T_FUNCTION, 0)
    local f2 = nbp.Node:new('F', nbp.T_FUNCTION, 2)
    local f4 = nbp.Node:new('F', nbp.T_FUNCTION, 4)
    local cn1 = nbp.Node:new('CN1', nbp.T_CONSTANT, 0)
    local cn2 = nbp.Node:new('CN2', nbp.T_CONSTANT, 0)
    ...
    for _, v in ipairs(list) do
        ...
        self.pList[#self.pList+1] = { v[1], root }
    end
end

function TestExportStructurePython:test_export()
    -- for k, v in ipairs(self.pList) do
        -- local str = v[1]
        -- local expected = v[2]
        -- local result = nbp.export_structure_python(str)
        -- lu.assertEquals(result, expected)
    -- end
end

-- class TestExportStructurePython
